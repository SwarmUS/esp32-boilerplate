...
bool MessageDispatcher::dispatchNetworkAPI(const MessageDTO& message,
    ...
    if (NetworkApiDTO* response = std::get_if<NetworkApiDTO>(&reply)) {
        if (std::holds_alternative<std::monostate>(response->getApiCall())) {
            // If it returns std::monostate, no response needs to be sent
        ...
        // std::monostate
        MessageDTO msg(m_bsp.getHiveMindUUID(), message.getSourceId(), *response);
