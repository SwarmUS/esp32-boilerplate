...
std::variant<ErrorNum, NetworkApiDTO> NetworkAPIHandler::handleApiCall(
    ...
    if (const auto* ipRequest = std::get_if<IPDiscoveryDTO>(&call)) {
        ...
        return std::variant<ErrorNum, NetworkApiDTO>(NetworkApiDTO());
    }

    // Return an empty optional to signal an error occurred while handling apiCall.
    return std::variant<ErrorNum, NetworkApiDTO>(ErrorNum::UNKNOWN_CALL);
}